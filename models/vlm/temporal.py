...
class TemporalUnet(nn.Module):
    ...
    def forward(self, x, neighbor_x, time, returns=None, reward_mask=None, drop_return=False):
        '''
        ...
        returns = einops.rearrange(returns, 'b h 1-> b h')
        reward_mask = einops.rearrange(reward_mask, 'b h 1-> b h')

        t = self.time_mlp(time)

        if self.returns_condition:
            assert returns is not None
            returns = returns * reward_mask
    ...
    def get_pred(self, x, neighbor_x, time, returns=None, reward_mask=None, drop_return=False):
        '''
            x : [ batch x horizon x transition ]
            returns : [batch x horizon]
        '''
        ...
        if self.returns_condition:
            assert returns is not None
            returns_embed = self.returns_mlp(returns)
            returns_embed = returns_embed * reward_mask
            if self.training:
                mask = self.mask_dist.sample(sample_shape=(returns_embed.size(0), 1)).to(returns_embed.device)
                returns_embed = mask*returns_embed
            if drop_return:
