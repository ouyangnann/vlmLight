...
class Intersection:
    def __init__(self, inter_id, dic_traffic_env_conf, eng, light_id_dict, path_to_log, lanes_length_dict):
        ...
        self.dic_lane_vehicle_previous_step_out = {}
        ...
        self.dic_lane_vehicle_current_step_out = {}
        ...
        self.list_lane_flow_in = []
        self.list_lane_flow_out = []
    ...
    def update_previous_measurements(self):
        ...
        self.dic_lane_vehicle_previous_step_out = self.dic_lane_vehicle_current_step_out
    ...
    def update_current_measurements(self, simulator_state):
        ...
        for lane in self.list_exiting_lanes:
            self.dic_lane_vehicle_current_step_out[lane] = simulator_state["get_lane_vehicles"][lane]
        ...
        self.list_lane_vehicle_current_step_out = _change_lane_vehicle_dic_to_list(self.dic_lane_vehicle_current_step_out)
        self.list_lane_vehicle_previous_step_out = _change_lane_vehicle_dic_to_list(self.dic_lane_vehicle_previous_step_out)
        ...
        # get lane level vehicle info
        if not self.list_lane_vehicle_previous_step_in:
            list_vehicle_lane_new_arrive_in = [len(lane_vehicle) for lane_vehicle in list(self.dic_lane_vehicle_current_step_in.values())]
        else:
            list_vehicle_lane_new_arrive_in = [len(
                        set(list(self.dic_lane_vehicle_current_step_in.values())[i]) -
                        set(list(self.dic_lane_vehicle_previous_step_in.values())[i])
                    ) for i in range(len(self.list_entering_lanes))]
            
        if not self.list_lane_vehicle_previous_step_out:
            list_vehicle_lane_new_arrive_out = [len(lane_vehicle) for lane_vehicle in list(self.dic_lane_vehicle_current_step_out.values())]
        else:
            list_vehicle_lane_new_arrive_out = [len(
                        set(list(self.dic_lane_vehicle_current_step_out.values())[i]) -
                        set(list(self.dic_lane_vehicle_previous_step_out.values())[i])
                    ) for i in range(len(self.list_exiting_lanes))]

        # update flow
        if len(self.list_lane_flow_in) == 0 or \
                len(self.list_lane_flow_out) == 0:
            self.list_lane_flow_in = [0] * len(self.list_entering_lanes)
            self.list_lane_flow_out = [0] * len(self.list_exiting_lanes)
        else:
            if self.get_current_time() % self.dic_traffic_env_conf["MIN_ACTION_TIME"] == 1:
                # clear the list
                self.list_lane_flow_in = [0] * len(self.list_entering_lanes)
                self.list_lane_flow_out = [0] * len(self.list_exiting_lanes)

            self.list_lane_flow_in = [self.list_lane_flow_in[i] + list_vehicle_lane_new_arrive_in[i] for i in range(len(self.list_entering_lanes)) ]
            self.list_lane_flow_out = [self.list_lane_flow_out[i] + list_vehicle_lane_new_arrive_out[i] for i in range(len(self.list_exiting_lanes)) ]
    ...
    def _update_feature(self):
        ...
        tmp_part_n, tmp_part_q, tmp_efficient_part, enter_running_part, lane_queue_in_part, lane_queue_out_part = self._get_part_traffic_movement_features()

        # dic_feature["lane_enter_running_part"] = list(enter_running_part)

        # -------- queue vehicles------------
        dic_feature["lane_queue_in_part"] = list(lane_queue_in_part)
        dic_feature["lane_queue_out_part"] = list(lane_queue_out_part)
        # -------- run vehicle --------------
        dic_feature["lane_run_in_part"] = list(enter_running_part)

        dic_feature["lane_flow_in"] = self.list_lane_flow_in
        dic_feature["lane_flow_out"] = self.list_lane_flow_out
        ...
        dic_feature["num_in_seg"] = self._orgnize_several_segments2()

        if self.current_phase_index >= 0:
            dic_feature["new_phase"] = self.dic_traffic_env_conf['PHASE'][self.current_phase_index]
        else:
            dic_feature["new_phase"] = [0] * len(self.dic_traffic_env_conf['PHASE'][1])
    ...
    def _get_part_traffic_movement_features(self):
        ...
        return traffic_movement_pressure_nums, tmp_queue_part, tmp_queue_efficient_part, part_entering_running, list_entering_part_queue, list_exiting_part_queue
